...
function usbbulk(){
    ...
    self.start = function(vid,pid){
        var device,cdc,custom;
            vid = vid||0x1209
            pid = pid||0x0D32
            device = usb.findByIds(0x1209,0x0D32)// Generic, ODrive Robotics ODrive v3
            if(!device){
                ...
                cdc.in     = findEp(cdc , usb.usb.LIBUSB_TRANSFER_TYPE_BULK, 'in');
                cdc.out    = findEp(cdc , usb.usb.LIBUSB_TRANSFER_TYPE_BULK, 'out');
                ...
                custom.in     = findEp(custom , usb.usb.LIBUSB_TRANSFER_TYPE_BULK, 'in');
                custom.out    = findEp(custom , usb.usb.LIBUSB_TRANSFER_TYPE_BULK, 'out');
