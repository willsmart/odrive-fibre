...
function fibre() {
  ...
  function Response(data) {
    ...
    deb && console.log({response: buffer.toString()})
    ...
    if (sequence !== operation.sequence) {
      const error = new Error(sequence + '!=' + operation.sequence);
      self.emit('error', error);
      operation.errorCallback?.(error);
      return;
    }
    if ((queue[sequence].id & 0x7fff) !== operation.id) {
      const error = new Error(queue[sequence].id + '!=' + operation.id);
      self.emit('error', error);
      operation.errorCallback?.(error);
      return;
    }
    ...
    if (operation.id === 0) {
      if (payload.length > 0) {
        ...
        buf.writeUInt16BE(calc_crc16(PROTOCOL_VERSION, operation.payload));
        self.set_json_crc(buf.readUInt16BE(0));
  ...
  function Request(operation) {
    ...
    if ((id & 0x7fff) === 0) {
      ...
      trailer.writeUInt16LE(operation.crc());
    }
    queue[operation.sequence] = operation;
    return Buffer.concat([header, payload, trailer]);
  }
  function get_endpoint(id, size, callback, errorCallback) {
    deb && console.log({_:'get_endpoint',id, size, callback, errorCallback})
    ...
    function timeout() {
      if (operation.acked === false) {
        ...
        isReady = true;
        deb && console.log({get_endpoint_error: 'Ack timeout id: ' + id + '. Check CRC or fibre connection.'})
  ...
  function set_endpoint(id, payload, callback, errorCallback) {
    var operation = new packet();
    operation.id = id - 0;
